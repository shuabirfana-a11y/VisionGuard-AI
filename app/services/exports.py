...
def report_html(result: AnalysisResponse) -> str:
    targets = {item.evidence_id: f"visual-{index}" for index, item in enumerate(result.vision.detections)}
    if result.vision.fire_classification and result.vision.fire_classification.available:
        targets[result.vision.fire_classification.evidence_id] = "classifier-evidence"
    for index, item in enumerate(result.knowledge):
        targets[item.rule_id] = targets[item.citation_id] = f"knowledge-{index}"

    def reference_link(reference: str) -> str:
        if reference in targets:
            return f'<a href="#{targets[reference]}">{escape(reference)}</a>'
        return escape(reference)

    def source_link(url: str | None, title: str) -> str:
        try:
            parsed = urlparse(url or "")
            safe = parsed.scheme.lower() in {"http", "https"} and bool(parsed.netloc)
        except ValueError:
            safe = False
        return f'<a href="{escape(url, quote=True)}" rel="noopener noreferrer">{escape(title)}</a>' if safe else escape(title)
    ...
    evidence = "".join(
        f'<li id="visual-{index}"><b>{escape(item.evidence_id)}</b>：{escape(item.label)}，置信度 {item.confidence:.2%}，'
        f"位置 ({item.bbox.x1}, {item.bbox.y1})-({item.bbox.x2}, {item.bbox.y2})</li>"
        for index, item in enumerate(result.vision.detections)
    )
    classification = result.vision.fire_classification
    if classification and classification.available:
        evidence += (
            f'<li id="classifier-evidence"><b>{escape(classification.evidence_id)}</b>：整图可见火焰判断'
            f"{'阳性' if classification.prediction else '阴性'}，概率 {classification.probability:.2%}，"
            "不提供检测框</li>"
        )
    evidence = evidence or "<li>未获得明确视觉风险证据。</li>"
    citations = "".join(
        f'<li id="knowledge-{index}">[{escape(item.citation_id)}] '
        + source_link(item.source_url, item.source_title)
        + f" / {escape(item.source_section)} / {escape(item.source_version)}"
        + f"<p>{escape(item.basis)}</p>"
    ...
    trace = "".join(
        ...
        + (
            "；引用：" + "、".join(reference_link(reference) for reference in item.references)
            if item.references
            else ""
        )
        + "</small></div></li>"
    ...
    image_info = result.vision.input_image
    image_meta = (
        f'<div class="wide"><b>输入图片SHA-256：</b>{escape(image_info.sha256)}</div>'
        f'<div class="wide"><b>像素坐标：</b>{result.vision.image_width} × {result.vision.image_height}，'
        f'{"已校正拍摄方向" if image_info.orientation_corrected else "无需旋转"}；'
        f'{"透明区域合成白底" if image_info.transparency_composited else "RGB输入"}；'
        f'{escape(image_info.processing)}</div>'
        if image_info else ""
    )
    mode_notice = (
        '<p class="boundary">本报告使用演示视觉模式，仅用于体验操作流程，不能作为专业模型效果证明。</p>'
        if result.vision.inference.backend != "yolo" or result.vision.inference.fallback_used else ""
    )
...
.meta .wide{{grid-column:1/-1;overflow-wrap:anywhere}}li:target{{outline:2px solid #2e74b5;background:#eef4fa}}.level{{display:inline-block;font-size:24px;font-weight:700;color:#9b3518;border-left:5px solid #d96032;padding:5px 12px;background:#fff3ed}}
...
</style><script src="/static/report.js" defer></script></head><body>
<button id="print-report" type="button">打印 / 另存为PDF</button>
...
<div><b>推理器：</b>{escape(result.reasoning.provider)} / {escape(result.reasoning.model)}</div><div><b>回退状态：</b>视觉 {visual_fallback} / 推理 {reasoning_fallback}</div>{image_meta}</div>
...
<h2>可信推理</h2><p>{escape(result.reasoning.explanation)}</p>
<p>视觉引用：{'、'.join(reference_link(value) for value in result.reasoning.evidence_ids) or '无'}<br>
知识引用：{'、'.join(reference_link(value) for value in result.reasoning.knowledge_ids) or '无'}</p>
<div class="boundary">{escape(result.reasoning.safety_boundary)}</div>
