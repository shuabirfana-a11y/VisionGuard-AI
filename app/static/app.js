...
function renderResult(data) {
  ...
  detections.innerHTML = data.vision.detections.length
    ? data.vision.detections.map(item => `
      <div class="evidence">
  ...
  document.querySelector("#knowledge").innerHTML = data.knowledge.length
    ? data.knowledge.map(item => `
      <div class="evidence knowledge-card">
        ...
        <div>${escapeHtml(item.source_title)} / ${escapeHtml(item.source_section)}</div>
        <small>版本 ${escapeHtml(item.source_version)} · 检索相关度 ${(item.retrieval_score * 100).toFixed(1)}%</small>
      </div>`).join("")
